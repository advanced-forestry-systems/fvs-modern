...
PROJECT_ROOT <- "/users/PUOM0008/crsfaaron"; N_PLOTS <- 300L
OUT_DIR <- file.path(PROJECT_ROOT, "acadgy_fia_verify"); set.seed(2028)
...
rows <- list()
cat("[v36] csi_scale_0.7  (12.3.9, MORTCAL on, CutPoint 0, CSI_SCALE = 0.7; production posture)
")
rows[["a"]] <- summ("csi_scale_0.7",  run_cfg(TRUE, "Y", 0, 0.7))
cat("[v36] csi_scale_1.0  (12.3.9, MORTCAL on, CutPoint 0, no CSI_SCALE; baseline)
")
rows[["b"]] <- summ("csi_scale_1.0",  run_cfg(TRUE, "Y", 0, NULL))
res <- dplyr::bind_rows(rows)
write.csv(res, file.path(OUT_DIR, "acadgy_v33v34test_v36_results.csv"), row.names=FALSE)
