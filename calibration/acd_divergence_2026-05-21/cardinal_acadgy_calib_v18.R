...
source("/users/PUOM0008/crsfaaron/AcadianGY_12.3.8.r")  # IN-SOURCE model
...
cal <- read.csv(file.path(PROJECT_ROOT,"acd_annual_calibration_test.csv"))
...
p1y <- function(trees, mortcal, calib=FALSE, cutpoint=NULL) {
  pc <- list()
...
run_cfg <- function(mortcal, calib=FALSE, cutpoint=NULL) {
  cur <- base_init; bl <- list(); tl <- list(); ql <- list()
  bl[["0"]] <- basnap(cur); tl[["0"]] <- tpsnap(cur); ql[["0"]] <- qmsnap(cur)
  for (yr in 1:n_years) {
    cur <- p1y(cur, mortcal, calib, cutpoint); if (is.null(cur)) break
