...
WARM <- as.integer(get_arg("warmup", NA_character_)); SAMP <- as.integer(get_arg("sampling", NA_character_))
HOLDOUT_SPCD_FILE <- get_arg("holdout_spcd_file", NA_character_)

dir.create(OUT_DIR, recursive = TRUE, showWarnings = FALSE)
cat("== 32_fit_hg_speciesfree_v5.R ==\n")

DATA_FILE   <- get_arg("data", "data/conus_remeasurement_pairs_metric_cond_v2.rds")
TRAITS_FILE <- get_arg("traits", "traits/species_traits.rds")
...
MIN_OBS_SPECIES <- as.integer(get_arg("min_obs_species", "5000"))
...
if (grepl("cspiv6", STAN_FILE)) {
  cs <- as.numeric(dat$cspi_v6)
  med <- median(cs, na.rm=TRUE); sdv <- sd(cs, na.rm=TRUE)
  cs[!is.finite(cs)] <- med
  stan_data$cspi_v6_z <- (cs - med) / pmax(sdv, 0.1)
  cat("cspiv6: added cspi_v6_z (n=", length(stan_data$cspi_v6_z), ", range", round(range(stan_data$cspi_v6_z),2), ")\n")
}
...
if (SMOKE) {
  ...
  iter_warmup <- if(!is.na(WARM)) WARM else 1000; iter_sampling <- if(!is.na(SAMP)) SAMP else 1000; chains <- 4
}
...
vars <- intersect(unique(c(vars,"a2_quad","a_bard","a_blrd")), fit$metadata()$stan_variables)
...
save_re_hg <- function(zname, lv, tag) { s <- tryCatch(fit$summary(variables=zname, "mean","sd", ~quantile(.x, c(0.05,0.95))), error=function(e) NULL); if (is.null(s) || nrow(s) != length(lv)) return(invisible(NULL)); names(s)[names(s) %in% c("5%","95%")] <- c("q5","q95"); data.table::fwrite(data.frame(level=lv, mean=s$mean, sd=s$sd, q5=s$q5, q95=s$q95), file.path(OUT_DIR, paste0(OUT_NAME, "_re_", tag, ".csv"))) }
save_re_hg("z_L1", L1_levels, "L1"); save_re_hg("z_L2", L2_levels, "L2"); save_re_hg("z_L3", L3_levels, "L3"); save_re_hg("z_FT", FT_levels, "FT"); save_re_hg("z_sp", sp_levels, "sp")
