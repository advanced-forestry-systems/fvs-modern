...
## 5b. Scenario buckets + coexistence with the DB->API regeneration

FVS engines expose the canonical scenario taxonomy: `reserve (no harvest)`,
`managed (harvest)` (realistic, intensive-on-plantations), `managed (intensive)`
(all-intensive bound), `managed (conservation)` (lightest; internally "extensive").
The disturbance-exposed / mortality-stressed bucket variants are produced by the
master pipeline for other engines, not FVS.

Operational note: the three FVS national models are **direct-injected** into
`public/api/series/*.json` by `fvs_perseus_merge.py`, not generated by the master
database->API pipeline. They have survived the master regenerations to date (the
regen is additive by model id), but a full series rebuild from the database would
