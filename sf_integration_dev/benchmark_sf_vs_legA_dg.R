...
CKEY <- c(hg="height_growth",dg="diameter_growth",hcb="height_crown_base",
          cr="crown_recession",cr2="crown_recession",htdbh="height_diameter",mort="mortality")[[COMPONENT]]
...
# prep_cr2: CR2-direct redesign (logit(CR2) ~ b0 + b_cr1 logit(CR1) + b1 DBH +
# b2 DBH^2 + b3 BA + b4 BAL + b6 ln_csi). Response on CR2 scale; logit link.
prep_cr2 <- function(dat){
  dat[, resp:=CR2]; dat[, cr1_logit:=logit(pmin(pmax(CR1,1e-4),1-1e-4))]
  dat[, dbh:=DBH1]; dat[, dbh_sq:=DBH1^2]
  dat[, ba_metric:=BA1*0.2296]; dat[, bal_metric:=BAL_SW1+BAL_HW1]
  dat[, ln_csi:=ln_csi_of(dat)]; dat[!is.finite(ln_csi),ln_csi:=0]
  dat <- dat[is.finite(DBH1)&DBH1>=2.54 & is.finite(CR1)&CR1>0&CR1<1 &
    ...
    EPA_L1_CODE!=""&EPA_L2_CODE!=""&EPA_L3_CODE!="" & !is.na(FORTYPCD_cond1)&FORTYPCD_cond1>0]
  list(dat=dat,
       gfun=function(y) logit(pmin(pmax(y,1e-4),1-1e-4)),
       hfun=function(e) inv_logit(e),
       valid=function(y) y>1e-4 & y<1-1e-4,
       cov=function(d,f) f[["b_cr1"]]*d$cr1_logit + f[["b1"]]*d$dbh + f[["b2"]]*d$dbh_sq +
         f[["b3"]]*d$ba_metric + f[["b4"]]*d$bal_metric + f[["b6"]]*d$ln_csi,
       interval=function(e,d,f){ s<-f[["sigma"]]
         list(l=inv_logit(e-1.96*s), u=inv_logit(e+1.96*s)) })
}
...
PREP <- list(hg=prep_hg, dg=prep_dg, cr=prep_cr, cr2=prep_cr2, hcb=prep_hcb, htdbh=prep_htdbh)
