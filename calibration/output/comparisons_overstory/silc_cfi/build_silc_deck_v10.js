...
// ===================================================================
{
  const s = pres.addSlide();
  s.background = { color: "FFFFFF" };
  s.addText("100-yr AGM trajectories: default vs MORTCAL", {
    x: 0.5, y: 0.35, w: 12.3, h: 0.75,
    fontFace: "Aptos Display", fontSize: 32, color: CHARCOAL, bold: true,
    align: "left", valign: "middle"
  });
  s.addText("Default AGM (green) projects BA climbing to 200 ft^2/ac by year 100. MORTCAL=TRUE (gold) plateaus at 80-110 ft^2/ac - a more operationally credible carrying capacity for these stands.", {
  ...
  citationLine(s, "AGM v12.3.9 on 11 SILC byStrata stands; 4 cells have no byStrata coverage (Cedar A+B, Mixedwood A+B, both Other Softwood)");
  takeawayBand(s, "Default AGM over-projects standing BA by ~2x at year 100; MORTCAL corrects this");
}
...
// ===================================================================
{
  ...
  // Year-100 4-model outcomes (AGM default, AGM MORTCAL, FVS-NE default, FVS-NE calibrated, OSM-ACD)
  const HDRY = { bold: true, color: "FFFFFF", fill: { color: CRSF_GREEN } };
  const rows = [
    [{ text: "Stratum",          options: HDRY },
     { text: "n",                options: HDRY },
     { text: "AGM def",          options: HDRY },
     { text: "AGM MORTCAL",      options: HDRY },
     { text: "FVS-NE def",       options: HDRY },
     { text: "FVS-NE cal",       options: HDRY },
     { text: "OSM-ACD",          options: HDRY },
     { text: "AGM mc Cords",     options: HDRY },
...
// ===================================================================
{
  ...
  const rows = [
    ...
    ["CSI (AGM, FVS-ACD)",        "12.0 m", "15.78 m", "CSI_2030.tif @ lat/long",
  ...
  takeawayBand(s, "Refined inputs change rankings minimally; AGM MORTCAL still anchors low");
}
...
// ===================================================================
{
  ...
  // Three rec cards
  const recs = [
    { title: "AGM with MORTCAL correction anchors the operational floor",
      body: "Cuts 10 yr empirical BA bias from +18.5% to +7.8%; year 100 BA settles at 98 to 108 ft^2/ac and Cords at 35 to 39 cords/ac. Sawlog BdFt R^2 = 0.84 with single digit bias. Use as the long horizon operational primary for sawlog and yield.",
      accent: CRSF_GREEN },
    { title: "FVS-NE calibrated brackets the upper bound",
      body: "Year 100 BA 125 to 225 ft^2/ac, Cords 44 to 70. Sits between AGM default and MORTCAL on every cell. Use as the FVS variant cross check; pair with AGM MORTCAL for the operational range.",
      accent: CRSF_GREEN },
    { title: "FVS-NE calibrated stays best on short horizon BA bias",
      body: "Lowest 5 yr bias (-0.3% BA, -3.8% cords). Pair with AGM MORTCAL for cross-check on 5 yr operational plans; both share the merch volume formula.",
      accent: CRSF_GREEN },
...
// ===================================================================
{
  ...
  const steps = [
    ...
    { n: "2", t: "Backfill the four empty cells with plot level AGM MORTCAL", b: "Cedar/A+B, Mixedwood/A+B, both Other Softwood; pull from individual SILC CFI plots assigned to those cells, run AGM MORTCAL 100 yr" },
    ...
    { n: "4", t: "Set the uncertainty bands and ship the operational handoff", b: "Use AGM MORTCAL RMSE 10.7 ft^2/ac BA at 10 yr as the per plot uncertainty. Year 100 cords/ac estimates: 36 to 39 across strata" }
  ];
...
const out = `${CFI_DIR}/SILC_4Model_Benchmark_v10_Weiskittel.pptx`;
