...
CCH_MODE  <- get_arg("cch_mode", "start")
...
dat <- dat[
  ...
  is.finite(cch) & is.finite(CCH2) & CCH2 >= 0 &
...
## ---- CCH parameterization mode (start vs period-mean) -------------------
if (identical(CCH_MODE, "mean")) {
  dat[, cch := (CCH1 + CCH2) / 2]
  cat("CCH_MODE = mean: cch = (CCH1 + CCH2)/2\n")
} else {
  dat[, cch := CCH1]
  cat("CCH_MODE = start: cch = CCH1\n")
}
cat("cch (post-mode) range: [", round(min(dat$cch), 3), ",", round(max(dat$cch), 3), "]\n\n")
...
fit <- mod$sample(data = stan_data, chains = ch, parallel_chains = ch,
                  ...
                  adapt_delta = 0.95, max_treedepth = 11, seed = 2026, refresh = 100)
