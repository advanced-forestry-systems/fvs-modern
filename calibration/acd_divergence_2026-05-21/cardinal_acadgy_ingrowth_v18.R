...
run_cfg <- function(mortcal, ingrowth="Y") {
  cur <- base_init; bl <- list(); tl <- list(); ql <- list()
  bl[["0"]] <- basnap(cur); tl[["0"]] <- tpsnap(cur); ql[["0"]] <- qmsnap(cur)
  for (yr in 1:n_years) {
    cur <- p1y(cur, mortcal, ingrowth); if (is.null(cur)) break
...
rows <- list()
cat("[v18] ingrowth_on  (MORTCAL on, INGROWTH Y)\n"); rows[["on"]]  <- summ("ingrowth_on",  run_cfg(TRUE, "Y"))
cat("[v18] ingrowth_off (MORTCAL on, INGROWTH N)\n"); rows[["off"]] <- summ("ingrowth_off", run_cfg(TRUE, "N"))
res <- dplyr::bind_rows(rows); write.csv(res, file.path(OUT_DIR, "acadgy_ingrowth_v18_results.csv"), row.names=FALSE)
cat("\n=== FIA: ingrowth contribution to the residual BA over-projection ===\n"); print(format(res, digits=4))
