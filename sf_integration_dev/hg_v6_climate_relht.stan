...
data {
  ...
  vector[N_obs] bgi;
  vector[N_obs] climate_si_z;            // climate site index, z-scored
...
parameters {
  ...
  real a9a; real a9b;
  real a10;
  real k_climate;                       // climate modulator on species ceiling
...
transformed parameters {
  ...
  for (i in 1:N_obs) {
    real ceil_i = fmax(max_ht_sp_m[sp_idx[i]] * (1.0 + k_climate * climate_si_z[i]), 0.1);
    real r = ht_obs_m[i] / ceil_i;
    relht[i] = r;
