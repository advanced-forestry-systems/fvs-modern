...
FT2M = 0.3048  # engine HT is in feet; report metric HT to match conus projector

def stand_metrics(tl: pd.DataFrame) -> dict:
    nan = float("nan")
    if tl is None or tl.empty:
        return {"BA_FT2AC": 0.0, "QMD_IN": 0.0, "TPH": 0.0, "HT_M_MEAN": nan, "HT_M_DOM": nan}
    # vectorized columns
    dbh = pd.to_numeric(tl.get("DBH", tl.get("Dbh")), errors="coerce")
    tpa = pd.to_numeric(tl.get("TPA", tl.get("Tpa", 1.0)), errors="coerce")
    ht  = pd.to_numeric(tl.get("HT", tl.get("Ht", tl.get("TotHt"))), errors="coerce")
    m = dbh.notna() & (dbh >= 1.0) & tpa.notna() & (tpa > 0)
    dbh = dbh[m].to_numpy(float); tpa = tpa[m].to_numpy(float); ht = ht[m].to_numpy(float)
    if dbh.size == 0 or tpa.sum() <= 0.0:
        return {"BA_FT2AC": 0.0, "QMD_IN": 0.0, "TPH": 0.0, "HT_M_MEAN": nan, "HT_M_DOM": nan}
    sum_ba_in2 = float((dbh*dbh*tpa).sum()); sum_tpa = float(tpa.sum())
    ba = BA_CONST * sum_ba_in2
    qmd = np.sqrt(sum_ba_in2 / sum_tpa)
    tph = sum_tpa * TPA_PER_HA
    # real HT (feet -> meters)
    okh = np.isfinite(ht) & (ht > 0)
    htm = nan; htd = nan
    if okh.any():
        hh = ht[okh]*FT2M; tt = tpa[okh]; dd = dbh[okh]
        htm = float((hh*tt).sum()/tt.sum())
        # top height: TPA-wtd mean HT of largest-DBH stems summing to 100 stems/ha (=100/2.4710538 tpa)
        order = np.argsort(-dd)
        hs = hh[order]; ts = tt[order]
        target = 100.0/TPA_PER_HA
        cum = np.cumsum(ts)
        ndom = int(np.searchsorted(cum, target) + 1)
        ndom = min(ndom, len(ts))
        w = ts[:ndom].copy()
        if ndom >= 1:
            w[-1] = w[-1] - (cum[ndom-1] - target)
            w = np.clip(w, 0, None)
            if w.sum() > 0:
                htd = float((hs[:ndom]*w).sum()/w.sum())
    return {"BA_FT2AC": round(float(ba),4), "QMD_IN": round(float(qmd),4), "TPH": round(float(tph),4),
            "HT_M_MEAN": (round(htm,4) if np.isfinite(htm) else nan),
            "HT_M_DOM":  (round(htd,4) if np.isfinite(htd) else nan)}
                ...
                try:
                    ...
                    for cy, tl in sorted(fr["treelists"].items()):
                        ...
                        rows.append({"STAND_CN": cn, "STATE": state, "YEAR": cy, "PROJ_YEAR": py,
                                     ...
                                     "HT_M_MEAN": sm["HT_M_MEAN"], "HT_M_DOM": sm["HT_M_DOM"]})
                except Exception as e:
