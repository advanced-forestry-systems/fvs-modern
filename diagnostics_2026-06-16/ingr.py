...
A={a:{x:[] for x in ["BA","TPH","oBA","oTPH"]} for a in ["default","NOAUTOES"]}; n=0
for _,r in rem.iterrows():
    ...
    for lab,kw in [("default",""),("NOAUTOES","NOAUTOES")]:
        s=run(std,tdf,sid,kw,yrs)
        if s is None or len(s)==0: continue
        l=s.iloc[-1]; ba=g(l,"BA")*M2HA
        if ba<=0: continue
        A[lab]["BA"].append(ba); A[lab]["TPH"].append(g(l,"Tpa")*TPHc); A[lab]["oBA"].append(oBA); A[lab]["oTPH"].append(oTPH)
print("n",n)
for lab in ["default","NOAUTOES"]:
    bb=stt(A[lab]["BA"],A[lab]["oBA"]); tt=stt(A[lab]["TPH"],A[lab]["oTPH"]); print("%-10s BA bias %5.1f%%  TPH bias %5.1f%%  n=%d"%(lab,bb[0],tt[0],bb[1]))
print("DONE_INGR")
