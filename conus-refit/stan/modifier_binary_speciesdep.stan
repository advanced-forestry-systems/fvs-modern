// ============================================================================
// modifier_binary_speciesdep.stan
//
// SPECIES-DEPENDENT (b2-analog) binary modifier for mortality/survival.
// Extends modifier_binary.stan (Bernoulli logit with the base-fit eta as an
// offset) by letting each disturbance/treatment alpha vary by species through
// a TRAIT-INFORMED per-species random effect, exactly as modifier_speciesdep
// does for the continuous-residual components:
//
...
//
// Setting all sigma_sp_k -> 0 and gamma_k -> 0 recovers the species-free binary
// modifier; the trait part carries rare species, the z_sp_k part lets
// well-sampled species (e.g. fire response of ponderosa pine) pull free. Tight
// partial-pooling sigma so sparse disturbance-by-species cells shrink. Emits
// log_lik for held-out ELPD vs the species-free binary modifier.
//
// Same data as modifier_binary.stan plus N_sp, sp_idx, W, P_trait.
// ============================================================================
data {
  int<lower=1> N_obs;
  int<lower=1> N_L1;
...
generated quantities {
  ...
  for (i in 1:N_obs) {
    ...
    real delta_i =
      ...
      + (alpha_siteprep + sp_siteprep[s]) * x_siteprep[i]
      + z_L1[L1_idx[i]];
    real e = fmin(fmax(eta_base[i] + log_years[i] + delta_i, -30.0), 30.0);
    log_lik[i] = bernoulli_logit_lpmf(alive[i] | e);
  }
}
