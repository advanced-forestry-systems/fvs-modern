...
dat[, cr2_logit := qlogis(pmin(pmax(CR2,0.01),0.99))]; dat[, cr1_logit := qlogis(pmin(pmax(CR1,0.01),0.99))]
...
stan_data <- list(
  ...
  cr2_logit = dat$cr2_logit, cr1_logit = dat$cr1_logit,
  dbh = dat$DBH1,
  dbh_sq = dat$DBH1^2,
...
if (SMOKE) {
  ...
  iter_warmup <- 1500; iter_sampling <- 1000; chains <- 4
}
...
fit <- mod$sample(
  ...
  seed = 42, adapt_delta = 0.95, max_treedepth = 12, refresh = 100
)
