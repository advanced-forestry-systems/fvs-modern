// modifier_gompit_traitmed.stan
// TRAIT-MEDIATED (common + trait) gompit disturbance modifier: each disturbance
// effect is a global alpha plus a trait projection W*gamma_alpha, but NO
// per-species random deviation z_sp. Nested below modifier_gompit_speciesdep
// (which adds z_sp) and above modifier_gompit_common (global alpha only).
// Same data interface and gompit likelihood as the species-dependent variant.
data {
  int<lower=1> N_obs;
  int<lower=1> N_L1;
  int<lower=1> N_sp;
...
transformed parameters {
  ...
  vector[N_sp] sp_plant    = W * gamma_alpha_plant;
  vector[N_sp] sp_fire     = W * gamma_alpha_fire;
  vector[N_sp] sp_insect   = W * gamma_alpha_insect;
  vector[N_sp] sp_disease  = W * gamma_alpha_disease;
  vector[N_sp] sp_wind     = W * gamma_alpha_wind;
  vector[N_sp] sp_harvest  = W * gamma_alpha_harvest;
  vector[N_sp] sp_cutting  = W * gamma_alpha_cutting;
  vector[N_sp] sp_siteprep = W * gamma_alpha_siteprep;
}
