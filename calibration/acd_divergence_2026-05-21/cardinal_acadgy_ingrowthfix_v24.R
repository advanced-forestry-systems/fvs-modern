...
rows <- list()
cat("[v24] baseline   (12.3.7, CutPoint 0.95, ~no ingrowth, MORTCAL on)\n"); rows[["a"]] <- summ("baseline",      run_cfg(TRUE, "Y", 0.95))
cat("[v24] ingrowthfix(12.3.7, CutPoint 0 expected-value, recruits grow)\n"); rows[["b"]] <- summ("ingrowth_fix",  run_cfg(TRUE, "Y", 0))
res <- dplyr::bind_rows(rows); write.csv(res, file.path(OUT_DIR, "acadgy_ingrowthfix_v24_results.csv"), row.names=FALSE)
cat("\n=== FIA: does the #127 ingrowth carry-through fix change QMD/BA? ===\n"); print(format(res, digits=4))
