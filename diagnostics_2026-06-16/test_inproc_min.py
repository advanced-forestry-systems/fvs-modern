#!/usr/bin/env python3
"""Minimal in-process FVS test against the freshly built .so (2026-06-18).
Uses an input-only keyword file (STANDSQL/TREESQL for input via SQLite; no DSNOUT / output-DB keywords,
since results are read through the summary API). Confirms (a) the in-process keyword reader works after the
rebuild, (b) the stand loads from the DB in process, (c) get_tree_attr reads per-tree state, and (d)
set_tree_attr('dg', dg*1.3) at stop point 5 changes the final basal area, proving in-engine injection."""
import os, sys, tempfile, sqlite3, shutil
P="/users/PUOM0008/crsfaaron/fvs-modern"; FIA="/fs/scratch/PUOM0008/crsfaaron/FIA"; CONUS="/users/PUOM0008/crsfaaron/fvs-conus"
os.environ.update(FIA_DATA_DIR=FIA,FVS_PROJECT_ROOT=P,FVS_LIB_DIR=P+"/lib",FVS_CONFIG_DIR=P+"/config")
for p in [P+"/deployment/fvs2py", os.path.expanduser("~/overthin_work"), CONUS+"/python", P]:
    sys.path.insert(0, p)
import pandas as pd, numpy as np
from pathlib import Path
import fia_stand_generator as G
from perseus_100yr_projection import build_fvs_standinit, build_fvs_treeinit
try:
    from fvs2py import FVS
except Exception:
    from fvs2py._base import FVS
LIB=os.environ.get("LIB", P+"/lib-test/FVSne.so"); VAR="ne"

INPROC_KEY = """STDIDENT
{stand_id}
DATABASE
DSNIN
{db_path}
STANDSQL
SELECT * FROM fvs_standinit WHERE stand_id = '%StandID%'
ENDSQL
TREESQL
SELECT * FROM fvs_treeinit WHERE stand_id = '%StandID%'
ENDSQL
END
TREELIST           0
TIMEINT            0{cycle_length:>10d}
NUMCYCLE{num_cycles:>12d}
{calibration_keywords}
PROCESS
STOP
"""

G.VARIANT_STATES[VAR]=(23,33,50)
plot=pd.read_csv(Path(FIA)/"ME_PLOT.csv",usecols=lambda c:c in ("CN","PREV_PLT_CN","MEASYEAR","STATECD"),low_memory=False)
yr=dict(zip(plot.CN.astype("int64"),plot.MEASYEAR))
rem=plot.dropna(subset=["PREV_PLT_CN"]).copy(); rem["PREV_PLT_CN"]=rem.PREV_PLT_CN.astype("int64"); rem["CN"]=rem.CN.astype("int64")
rem["interval"]=rem.apply(lambda r:r.MEASYEAR-yr.get(r.PREV_PLT_CN,np.nan),axis=1)
rem=rem[(rem.interval>=5)&(rem.interval<=15)].sample(n=40,random_state=5)
tr1=G.load_fia_trees(VAR,rem.PREV_PLT_CN.tolist(),Path(FIA))
sel=None
for _,r in rem.iterrows():
    a=tr1[tr1.PLT_CN==int(r.PREV_PLT_CN)]; al=a[(a.STATUSCD==1)&(a.DIA>0)]
    if len(al)>=10: sel=(int(r.PREV_PLT_CN),int(r.STATECD),al); break
assert sel; t1,sc,al=sel; sid=str(t1)

def run(inject):
    tmp=tempfile.mkdtemp(); db=os.path.join(tmp,"FVS_Data.db")
    std=build_fvs_standinit({"INVYR":2000,"STATECD":sc,"COUNTYCD":0},sid,VAR); std["inv_plot_size"]=1.0; std["brk_dbh"]=99.0
    tdf=build_fvs_treeinit(al,sid)
    c=sqlite3.connect(db); std.to_sql("fvs_standinit",c,if_exists="replace",index=False); tdf.to_sql("fvs_treeinit",c,if_exists="replace",index=False); c.close()
    kp=os.path.join(tmp,"t.key"); open(kp,"w").write(INPROC_KEY.format(stand_id=sid,db_path=db,cycle_length=10,num_cycles=2,calibration_keywords="** D"))
    fvs=FVS(LIB); fvs.load_keyfile(kp); fvs.run(stop_point_code=7)
    n0=fvs.dims["ntrees"]; dbh0=fvs.get_tree_attr("dbh") if n0>0 else np.array([])
    steps=0
    while fvs.itrncd==0:
        fvs.run(stop_point_code=5, stop_point_year=-1)
        if fvs.restart_code==5:
            if inject:
                dg=fvs.get_tree_attr("dg"); fvs.set_tree_attr("dg", dg*1.30)
            steps+=1
        if fvs.restart_code==100 or fvs.itrncd!=0: break
    while fvs.itrncd==0:
        fvs.run();
        if fvs.restart_code==100 or fvs.itrncd!=0: break
    summ=fvs.summary; shutil.rmtree(tmp,ignore_errors=True)
    bacol=next((c for c in (summ.columns if summ is not None else []) if str(c).upper()=="BA"),None)
    ba=float(summ[bacol].iloc[-1]) if (summ is not None and len(summ) and bacol) else float("nan")
    return n0, (float(dbh0.mean()) if len(dbh0) else float("nan")), steps, ba, (list(summ.columns) if summ is not None else None)

n0,dbhm,st,ba_b,cols=run(False)
_,_,_,ba_i,_=run(True)
print("ntrees in-process:",n0," meanDBH:",round(dbhm,2)," stop5 stops:",st)
print("summary cols:",cols)
print("final BA base:",round(ba_b,2)," injected(+30%% dg):",round(ba_i,2)," delta:",round(ba_i-ba_b,2))
print("INPROC_AND_INJECTION_WORK" if (n0>0 and ba_i>ba_b+0.01) else ("INPROC_LOADS_NO_INJECT_EFFECT" if n0>0 else "INPROC_LOAD_FAILED"))
print("DONE_INPROC_MIN")
