...
//
// Architecture variant 3: trait × site interaction. Each species' response
// to site productivity is modulated by its trait vector. Lets shade-tolerant
// species be less responsive to site than shade-intolerant species, etc.
//
// Linear predictor:
//   ln(dDBH_a) = b0 + trait_effect[sp] + z_L1 + z_L2 + z_L3
//              + b1 ln(DBH) + b2 DBH + b3 ln((CR+0.2)/1.2)
//              + b4 ln(BAL_SW+0.01) + b5 BAL_HW
//              + (b6 + species_site_slope[sp]) * ln(CSI)  <-- trait-modulated slope
//              + b7 BA x RD + b8 BAL x RD
//
// where species_site_slope[sp] = W[sp,] * gamma_site
// ============================================================================
data {
  int<lower=1> N_obs;
  int<lower=1> N_sp;
...
parameters {
  ...
  vector[P_trait] gamma;             // species-trait intercept loading
  vector[P_trait] gamma_site;        // species-trait site-slope loading <-- new
...
model {
  ...
  gamma      ~ normal(0, 0.5);
  gamma_site ~ normal(0, 0.3);       // tighter prior on trait-site interaction
  ...
  vector[N_obs] b_site = b6 + species_site_slope[sp_idx];
...
generated quantities {
  ...
  vector[N_obs] b_site_gq = b6 + species_site_slope[sp_idx];
  vector[N_obs] eta_gq =
      b0 + trait_effect[sp_idx]
    + z_L1[L1_idx] + z_L2[L2_idx] + z_L3[L3_idx]
    + b1 * ln_dbh + b2 * dbh + b3 * ln_cr_adj
