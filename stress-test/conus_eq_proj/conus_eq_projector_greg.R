#!/usr/bin/env Rscript
## conus_eq_projector_greg.R -- FAITHFUL "Greg fvs_remodeling" projection arm (CONFIG=greg).
## ----------------------------------------------------------------------------------------
...
fb_dg_acc<-0; fb_hg_acc<-0; fb_mo_acc<-0; tc_acc<-0; perstand_fb<-vector("list",nrow(si)); pj<-0
...
for(stcode in names(state_groups)){
  ...
  for(ix in state_groups[[stcode]]){
    ...
    ## per-stand fallback fraction (tree-count weighted)
    tcw<-tl$TPA; tct<-sum(tcw)
    sfb_dg<-sum(tcw[!tl$has_dg])/tct; sfb_hg<-sum(tcw[!tl$has_hg])/tct; sfb_mo<-sum(tcw[!tl$has_mo])/tct
    fb_dg_acc<-fb_dg_acc+sum(tcw[!tl$has_dg]); fb_hg_acc<-fb_hg_acc+sum(tcw[!tl$has_hg]); fb_mo_acc<-fb_mo_acc+sum(tcw[!tl$has_mo]); tc_acc<-tc_acc+tct
    pj<-pj+1; perstand_fb[[pj]]<-data.table(STAND_CN=cn,VARIANT=VARIANT,fb_dg=sfb_dg,fb_hg=sfb_hg,fb_mo=sfb_mo)
    ## seed missing HT via Greg HG curve integrated to a plausible ht? simpler: small init then let it grow.
    miss<-!is.finite(tl$HT); if(any(miss)) tl$HT[miss]<-4.5*tl$dbh_in[miss]^0.5+4.5   # crude ft seed for missing only
    emit<-function(cy){sm<-stand_metrics(tl); py<-cy*CYCLEN; ri<<-ri+1
      ...
      ti<<-ti+1; treerows[[ti]]<<-data.table(STAND_CN=cn,CONFIG=CONFIG,PROJ_YEAR=py,SPCD=tl$SPCD,DBH_IN=tl$dbh_in,HT_M=tl$HT*0.3048,TPA=tl$TPA)}
    emit(0)
    for(cy in 1:NCYC){
      ## cch per tree this cycle (cch_module expects HT in feet; CR frac; EXPAN TPA)
      gg<-grp_organon(tl$SPCD)
      cch<-stand_cch(tl$dbh_in, tl$HT, tl$CR, tl$TPA, gg); tl$cch_last<-cch
      ## ---- annual loop: DG, HG, mortality (Greg forms) over CYCLEN years ----
      surv_cyc<-rep(1.0,length(tl$dbh_in))
      for(yr in 1:CYCLEN){
        dgan<-dg_annual(tl$GSPCD, tl$dbh_in, tl$CR, tl$HT, tl$CCFL, elev, emt)   # bal proxy = CCFL=BAL ft2/ac
        ...
        ## within-cycle competition is held at cycle-start cch/CCFL/bal (recomputed each cycle, as v2)
      }
      ## FAITHFUL GREG: mortality is governed by Greg's gompit (cr+cch) ALONE. cch rises with
      ## density so competition-driven mortality is already captured by Greg's form. The
      ## fvs-conus SDImax 8x hazard ramp is NOT applied here (it double-counts density and is
      ## non-Greg; with it BA collapsed post-peak). A light cap engages ONLY above the physical
      ## SDImax ceiling (rd>1) to prevent unbounded density, ramping gently (max 2x at rd=1.5).
      ...
      ## crown update (fvs-conus CR-recession; non-Greg) using end-of-cycle HT
...
out<-rbindlist(rows[seq_len(ri)]); tre<-rbindlist(treerows[seq_len(ti)]); psf<-rbindlist(perstand_fb[seq_len(pj)])
