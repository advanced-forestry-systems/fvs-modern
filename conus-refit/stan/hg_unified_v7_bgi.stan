// ============================================================================
// hg_unified_v7_bgi.stan
//
// Height growth (ORGANON form) species-free B1, v7 architecture.
//
// COMBINES THREE INDEPENDENT IMPROVEMENTS:
//   1. Species random intercept (z_sp / sigma_sp) — from hg_unified.
//      Adds +1145.6 ELPD over v6 (z=132) per LOO compare 2026-05-27.
//   2. ln(HT)^2 quadratic asymptote (a2_quad) — from v6.
//      Adds +288.6 ELPD over v5 (z=80) per LOO compare 2026-05-26.
//   3. traits_v3 with CE+S+LAC decomposition (via driver).
//      Adds +60.6 ELPD over v2split (z=24) for HG per LOO compare 2026-05-24.
//
// Expected combined gain over original HG v5 v2split: ~1500 ELPD if effects
// are approximately additive (as v6+v3 were).
//
// Linear predictor:
//   ln(HTG_a) = a0 + trait_effect[sp] + z_sp[sp]
//             + z_L1 + z_L2 + z_L3 + z_FT
//             + a1 ln(DBH)
//             + a2 ln(HT) + a2_quad ln(HT)^2          [v6 asymptote]
//             + a3 ln_cr_adj
...
parameters {
  ...
  real a2_quad;                     // v7: NEW quadratic ln(HT) (from v6)
  real a3;
  real a4;
  ...
  vector[N_sp] z_sp_raw;            // v7: NEW species RE (from unified)
  vector[N_L1] z_L1_bgi_raw;
  real<lower=0> sigma_L1;
  real<lower=0> sigma_L2;
  real<lower=0> sigma_L3;
  real<lower=0> sigma_FT;
  real<lower=0> sigma_sp;           // v7: NEW species RE scale (from unified)
...
model {
  ...
  a1      ~ normal( 0.0, 0.5);
  a2      ~ normal(-0.3, 0.5);
  a2_quad ~ normal(-0.05, 0.05);     // concentrated negative (v6 found -0.195)
  a3      ~ normal( 0.5, 0.5);
  a4      ~ normal( 0.05, 0.1);
  ...
  vector[N_obs] eta =
    ...
    + a1 * ln_dbh
    + a2 * ln_ht + a2_quad * ln_ht_sq      // v7: asymptote restored
    + a3 * ln_cr_adj
    + b_site .* bgi_b1
