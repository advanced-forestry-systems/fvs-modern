...
################################################################################
# v12.3.6.r  (candidate; canonical v12.3.5 PLUS an in-source, opt-in,
#             size-dependent mortality correction -- task #126b)
#
# This file is byte-for-byte the canonical AcadianGY v12.3.5 model EXCEPT for
# one optional feature, integrated natively into the per-cycle mortality step
# of AcadianGYOneStand rather than bolted on as a post-run EXPF wrapper
# (the earlier AcadianGY_12.3.5_mortcal.r approach):
#
#   ops$MORTCAL = TRUE  -> after each annual cycle, multiply SURVIVOR EXPF by a
#                          size-dependent survival ratio keyed on post-growth
#                          DBH (inches), annualised over ops$MORTCAL_INTERVAL
#                          (default 5). Fresh ingrowth is NOT haircut (the
#                          improvement over the wrapper, which trimmed ingrowth).
#
# DEFAULT IS OFF. With ops$MORTCAL not TRUE this model is identical in behaviour
# to canonical v12.3.5.
#
# IMPORTANT CAVEATS (do not enable globally):
#   * CONDITION-SPECIFIC. The survival ratios are calibrated to Maine FIA ACD
#     remeasurement. On independent Canadian CFI (MAGPlot NB) the same
#     correction OVER-corrects (basal area already unbiased there); see #128.
#     Enable only for FIA-like Maine conditions, never as a blanket default.
#   * PARTIAL. On 200 Maine FIA plots it roughly halves the basal-area
#     over-projection (+15.4% -> +8.6%) and lifts R^2 (0.42 -> 0.48) but does
#     NOT close it. The residual is a quadratic-mean-diameter / small-tree
#     ingrowth composition effect, NOT mortality (#127); the proper next fix is
#     in AcadianGY's recruitment / ingrowth submodel, not more mortality tuning.
#
...
# ---------------------------------------------------------------------------
# #126b in-source size-dependent mortality correction (opt-in via ops$MORTCAL).
# Observed / AcadianGY 5-yr survival ratio by post-growth DBH class (inches),
# from the Maine FIA v10 survival-by-size diagnostic. CONDITION-SPECIFIC: do not
# enable outside FIA-like Maine conditions (over-corrects Canadian CFI, #128).
# ---------------------------------------------------------------------------
...
AcadianVersionTag = "AcadianV12.3.8"
...
## crown recession
 ...
 ## Mortality ####
  ...
  # ---- #126b in-source size-dependent mortality correction (opt-in) --------
  # Applied to SURVIVORS only (before ingrowth is bound in), keyed on post-growth
  # DBH and annualised over MORTCAL_INTERVAL so it compounds correctly across the
  # caller's annual cycle loop. OFF by default => identical to canonical v12.3.5.
  # CONDITION-SPECIFIC (#128) and only partial (#127); see file header.
  if (isTRUE(ops$MORTCAL)) {
    .mc_iv <- if (!is.null(ops$MORTCAL_INTERVAL)) ops$MORTCAL_INTERVAL else 5
    tree <- tree %>%
      dplyr::mutate(EXPF = EXPF * .mortcal_surv_ratio(DBH / 2.54)^(1 / .mc_iv),
                    EXPF = ifelse(EXPF < 0.00001, 0.00001, EXPF))
  }
