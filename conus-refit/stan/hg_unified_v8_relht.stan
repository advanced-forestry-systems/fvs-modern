// ============================================================================
// hg_unified_v8_relht.stan
//
// Height growth (ORGANON form) species-free, v8_relht architecture.
//
// SAME as v7 (species RE z_sp + traits_v3 + 3-piece BGI + ecoregion/FT REs)
// EXCEPT the asymptote mechanism:
//   v7:        a2 * ln(HT) + a2_quad * ln(HT)^2     (empirical quadratic)
//   v8_relht:  a2 * log(1 - HT / max_ht_sp)         (mechanistic species ceiling)
//
// Motivation: the relht ceiling is biologically grounded (growth shuts down as
// height approaches the species maximum) and the v6_relht smoke recovered the
// correct open-grown shape (juvenile peak then mature decline) and flipped the
// BAL coefficient to the correct negative sign. This variant tests whether the
// mechanistic ceiling beats the empirical quadratic on LOO when both sit on the
// full v7 architecture. Driver auto-supplies ht_obs_m + max_ht_sp_m for any
// stan whose filename contains "relht".
//
// Linear predictor:
//   ln(HTG_a) = a0 + trait_effect[sp] + z_sp[sp]
//             + z_L1 + z_L2 + z_L3 + z_FT
//             + a1 ln(DBH)
//             + a2 log(1 - HT/max_ht_sp)            [relht ceiling]
//             + a3 ln_cr_adj
...
data {
  ...
  // relht inputs (driver supplies when filename matches "relht")
  vector<lower=0>[N_obs] ht_obs_m;
  vector<lower=0>[N_sp] max_ht_sp_m;
...
model {
  a0      ~ normal(-1.0, 2.0);
  a1      ~ normal( 0.0, 0.5);
  a2      ~ normal( 0.5, 1.0);     // relht ceiling: asymptotic shutdown strength
  a3      ~ normal( 0.5, 0.5);
  a4      ~ normal( 0.05, 0.1);
  ...
  vector[N_obs] eta =
    ...
    + a1 * ln_dbh
    + a2 * log_one_minus_relht             // v8_relht: mechanistic ceiling
    + a3 * ln_cr_adj
    + b_site .* bgi_b1
