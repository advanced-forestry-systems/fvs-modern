...
!----------
!  GOMPIT OVERRIDE: replace the annual mortality rate RIP with Greg Johnson's
!  gompit annual rate (1 - exp(-H)) for fitted species; the period conversion
!  below then reproduces gompit period survival exactly. X=1 (gompit owns it).
!----------
IF(LGOMP .AND. GHAVE(ISPC)) THEN
  CRG=REAL(ICR(I))/100.0
  CCHG=CCHT(I)
  CALL GOMPSURV(ISPC,DBH(I),CRG,CCHG,1.0,SURVG)
  RIP=1.0-SURVG
  X=1.0
ENDIF
