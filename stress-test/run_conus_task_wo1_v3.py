#!/usr/bin/env python3
"""CONUS-wide FVS projection task (v3): SEEDING FIX on top of v2.

Defect (diagnosed 2026-06-21): perseus build_fvs_standinit sets inv_plot_size~6.0
and brk_dbh=5.0. With num_plots=1 FVS RE-EXPANDS the already-per-acre TREE_COUNT
(=TPA_UNADJ) by the small-tree plot factor (~6x) AND collapses trees above brk_dbh
to ~0 TPA, producing year-0 BA that is BOTH too high (small trees) and too low
(large trees dropped) -- net mean BA far below FIA-observed, with 28-49% of
far-West stands at BA=0. The fvs-conus equation arm treats TREE_COUNT directly
as per-acre TPA and matches FIA-observed stand BA to within 0.000%.

ARBITER (FIA-observed stand BA from treeinit, CR/PN/WC):
  observed = eq year0 BA exactly (100% of stands within 1%).
  engine v2 year0 BA = 0.44-0.82x observed; the ENGINE is wrong.

FIX: take supplied per-acre counts as-is. inv_plot_size=1.0, num_plots=1,
brk_dbh=999 (single per-acre design, no large-tree breakpoint). Verified on a
CR stand: FVS year-0 TPA/BA pass through unchanged (ratio 1.0000).

Everything else (dynamics, arms, cycles, treeinit build, AGB/BA/QMD/TPH
computation) is byte-for-byte identical to v2. Writes to out_conus_engine_v3.
Does NOT modify perseus_100yr_projection.py or any v2 output.
"""
...
def main():
    ...
    for state_fips, grp in si.groupby("STATE"):
        ...
        if not os.path.exists(tfile):
            failures.append({"stand_cn": None, "stage": "no_treeinit", "detail": state, "n": int(len(grp))}); continue
