...
source("/users/PUOM0008/crsfaaron/AcadianGY_12.3.8.r")  # #127 ingrowth-fix model
...
# v27c: load pre-extracted BGI from raster (see acadgy_fia_verify/extract_bgi.py).
# ME_BGI_V1.tif values are in a productivity scale (mean ~4091); recenter so
# mean of the matched sample equals 1.0 for use as a per-tree dDBH.mult.
...
bgi_per_tree <- bgi_per_plot[match(base_init$STAND, samp$PLT_CN_t1_str)]
bgi_per_tree[is.na(bgi_per_tree)] <- bgi_mean_pos
cat(sprintf("[v27c] BGI loaded: n=%d plots matched, mean=%.1f, median=%.1f\n",
    sum(!is.na(bgi_lookup[as.character(samp$PLT_CN_t1_str)])),
    bgi_mean_pos, median(bgi_per_plot, na.rm=TRUE)))
...
cat("[v27c] csi_baseline    (12.3.8, CutPoint 0 EV ingrowth, MORTCAL on, CSI = ClimateSI_ft)
")
rows[["a"]] <- summ("csi_baseline",    run_cfg(TRUE, "Y", 0))

stand_init <- stand_init_sicond
cat("[v27c] sicond_replace  (12.3.8, CutPoint 0 EV ingrowth, MORTCAL on, CSI = SICOND)
")
rows[["b"]] <- summ("sicond_replace",  run_cfg(TRUE, "Y", 0))

stand_init <- stand_init_fvssi
cat("[v27c] fvssi_replace   (12.3.8, CutPoint 0 EV ingrowth, MORTCAL on, CSI = FVS_SITE_INDEX)
")
...
write.csv(res, file.path(OUT_DIR, "acadgy_bgi_v27c_results.csv"), row.names=FALSE)
cat("
