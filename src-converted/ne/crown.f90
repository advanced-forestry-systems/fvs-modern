...
REAL CR,CL,HD,HN,CRMAX,CRLN,PDIFPY,CHG,DEN,D,DHTLC
DATA MYACTS/81/
!----------
...
!----------
59 CONTINUE
! Greg crown recession hook (ICRWDRV=1 when FVS_GREGCRW='1')
IF (ICRWDRV .EQ. 1 .AND. .NOT.LSTART .AND. ICR(I).GT.0 .AND. HTG(I).GT.0.0) THEN
    CL = HT(I) * REAL(ICR(I)) / 100.0
    CALL GREGCRWV(ISPC, CL, HTG(I), 0.0, DHTLC)
    IF (DHTLC .GT. 0.0) THEN
        CL = CL + HTG(I) - DHTLC
        IF (CL .LT. 0.0) CL = 0.0
        ICRI = MIN(ICRI, INT(CL / (HT(I) + HTG(I)) * 100.0 + 0.5))
    ENDIF
ENDIF
