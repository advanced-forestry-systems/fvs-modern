// build_silc_deck.js
// =====================================================================
// SILC operational deck for Ian / Ryan at Seven Islands Land Company.
// Committee Update mode, CRSF brand, focus on merch volume.
// =====================================================================
...
// ===================================================================
{
  ...
  s.addText("Mature Acadian stands reach 55 to 66 net cords/ac by year 100", {
    x: 0.5, y: 1.05, w: 12.3, h: 0.45,
    fontFace: "Aptos", fontSize: 16, color: MED_GRAY, italic: true,
    align: "left", valign: "top"
  });
  ...
  s.addImage({
    path: `${OD}/silc_strata_5x2_cords_deck.png`,
    x: 0.4, y: 1.55, w: 12.5, h: 4.40
  });

  citationLine(s, "AGM (AcadianGY) byStrata on 11 SILC matrix stands; 6 of 10 cells covered, 4 empty");
...
// ===================================================================
{
  ...
  const tab = [
    [{ text: "Predictor", options: HDR },
     ...
     { text: "Cords RMSE",     options: HDR },
     { text: "BdFt bias",      options: HDR },
     { text: "BdFt RMSE",      options: HDR }],
    ["zero growth",         "-1.8%", "7.1",  "-6.0%", "1.75", "-15.9%", "575"],
    ["FIA prior PAI",       "+3.9%", "7.0",  "+1.4%", "1.41", "n/a", "n/a"],
    ["AGM (AcadianGY)",
       "+6.1%", "7.6",
       "+7.0%", "1.82",
       { text: "+8.6%", options: { color: G, bold: true } },
       { text: "358", options: { color: G, bold: true } }],
    ["FVS-NE default",      "+4.6%", "8.1",  "-1.3%", "1.31", "n/a", "n/a"],
    ["FVS-NE calibrated",
    ...
    ["FVS-ACD default",
       "+5.3%", "8.4",
       "-2.5%",
       { text: "1.25", options: { color: G, bold: true } },
       "n/a", "n/a"],
    ["OSM-ACD",             "-8.0%", "8.9",  "-7.3%", "1.67", "-6.6%", "444"],
    ["units",
       { text: "%", options: { italic: true, color: MED_GRAY } },
       { text: "ft^2/ac", options: { italic: true, color: MED_GRAY } },
       { text: "%", options: { italic: true, color: MED_GRAY } },
       { text: "cords/ac", options: { italic: true, color: MED_GRAY } },
       { text: "%", options: { italic: true, color: MED_GRAY } },
       { text: "BdFt/ac", options: { italic: true, color: MED_GRAY } }]
  ];
  s.addTable(tab, {
    x: 0.3, y: 1.55, w: 12.7, h: 4.4,
    colW: [2.8, 1.4, 1.5, 1.6, 1.6, 1.4, 1.4],
  ...
  citationLine(s, "Routine growth subset n=17 (7 establishment pairs excluded); apples-to-apples 0.0025*DBH^2*HT volume formula across all predictors");
  takeawayBand(s, "FVS-NE calibrated wins BA, FVS-ACD wins cords, AGM wins sawlog BdFt");
}

// ===================================================================
// SLIDE 9: Operational interpretation
// ===================================================================
{
  const s = pres.addSlide();
  s.background = { color: "FFFFFF" };
...
// ===================================================================
{
  ...
  const steps = [
    ...
    { n: "2", t: "Add FVS sawlog BdFt via NVEL volume keyword", b: "FVS_Summary2 currently omits BdFt; enable NVEL to complete the four model BdFt scorecard" },
...
const out = `${CFI_DIR}/SILC_4Model_Benchmark_v2_Weiskittel.pptx`;
