program diag2
  ...
  real :: crg(3), cchmap(5), surv, s10
  character(len=512) :: header
  integer :: focus(3)
  ! realistic AFFINE-MAPPED cch values entering GOMPSURV (0.062 baseline .. denser)
  cchmap=(/0.062,0.20,0.50,1.00,2.50/)
  crg =(/0.20,0.45,0.80/)
  focus=(/12,94,241/)
  ...
  close(10)
  write(*,*) "SPCD cr cch_mapped surv_ann surv_10yr mort10_pct"
  do id=1,ns
    do k=1,3
      if(spcd(id)==focus(k)) then
        do ic=1,3
         do ih=1,5
          call gsurv(b0(id),b1(id),b2(id),b3(id),b4(id),crg(ic),cchmap(ih),surv,s10)
          write(*,900) spcd(id),crg(ic),cchmap(ih),surv,s10,(1.0-s10)*100.0
         end do
        end do
      end if
    end do
  end do
900 format(I5,2X,F5.2,2X,F7.3,2X,F9.6,2X,F9.6,2X,F7.2)
end program diag2
subroutine gsurv(B0,B1,B2,B3,B4,CR,CCHV,SURV,S10)
  implicit none
  real B0,B1,B2,B3,B4,CR,CCHV,SURV,S10
  ...
  HZ=1.0-EXP(-EXP(ETA))
  SURV=MAX(0.0,MIN(1.0,HZ)); S10=SURV**10.0
