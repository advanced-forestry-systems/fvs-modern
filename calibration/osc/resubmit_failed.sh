#!/bin/bash
#
# Resubmit failed variants (oc, tt, ws) with increased memory for rFIA Step 08
# These variants died OOM at 32G during stand data extraction.
# Also includes fixes for:
#   - Step 04/05 posterior::summarise_draws p95 column naming (scripts already updated)
#   - Step 03 H-D init value errors (non-fatal, acceptable)
#
# Usage: bash calibration/osc/resubmit_failed.sh
#
...
# Variants that need resubmission
FAILED_VARIANTS="oc tt ws"

# Generate the resubmission batch script
BATCH_SCRIPT="${SCRIPT_DIR}/_resubmit_failed_generated.sh"
...
echo "Submitting resubmission job..."
cd "$SCRIPT_DIR"
sbatch "$BATCH_SCRIPT"
echo "Done."
