...
source("/users/PUOM0008/crsfaaron/AcadianGY_12.3.7.r")  # #127 ingrowth-fix model
...
rows <- list()
cat("[v21] baseline   (12.3.7, CutPoint 0.95, ~no ingrowth, MORTCAL on)\n"); rows[["a"]] <- summ("baseline",      run_cfg(TRUE, "Y", 0.95))
cat("[v21] ingrowthfix(12.3.7, CutPoint 0 expected-value, recruits grow)\n"); rows[["b"]] <- summ("ingrowth_fix",  run_cfg(TRUE, "Y", 0))
res <- dplyr::bind_rows(rows); write.csv(res, file.path(OUT_DIR, "acadgy_ingrowthfix_v21_results.csv"), row.names=FALSE)
