#!/usr/bin/env Rscript
## conus_eq_projector_v6.R -- annualized stand projection using fvs-conus fitted equations,
## with the stand-level equations wired in to CONSTRAIN the tree-level predictions.
##
## conus_eq_projector_v6.R == v4 + HT-DBH MONOTONICITY GUARD (projection-time only).
##
...
##
## v4 CHANGES vs v3 (this file):
##   FIX A  DENSITY-DEPENDENT INGROWTH. v3 added a FIXED per-variant recruit rate (IG_TPA rec/ac/yr)
##          every cycle regardless of stocking -- closed stands kept recruiting into the overstory,
##          flooding stems while self-thinning removed them (TPH-up / QMD-down ingrowth signature).
##          v4 multiplies the recruit rate by a canopy-closure factor that tapers 1 -> 0 as relative
##          density RD=SDI/SDImax rises: f_ig = max(0, (1 - RD/IG_RDCAP))^IG_P, hard-off above IG_RDCAP.
##          Defaults IG_RDCAP=0.85, IG_P=1.5 (recruitment ~halts once the stand is ~0.6 of max SDI).
##   FIX B  SELF-THINNING RETUNE toward Reineke -1.6. v3 global ST_RATE=0.4525 ST_EXP=2.73 realized a
##          per-stand slope of ~-1.93 (slightly over-thinning) but correctly signed and uniform across
##          site classes. v4 default ST_RATE=0.34 ST_EXP=2.5 brings the realized slope toward -1.6.
##          Optional --st_site_scale lets the self-thin rate scale with site (off by default).
##   FIX C  SAVE HEIGHT. v4 writes TPA-weighted mean HT (HT_M_MEAN) and top/dominant HT (HT_M_DOM, the
##          TPA-weighted mean HT of the largest-DBH stems summing to 100 stems/ha) into the metrics CSV
##          so the Bakuzis/Eichhorn test uses REAL projected height, not a QMD-derived curve.
##
## v3 CHANGES vs v2:
##   ANNUALIZED. Everything is an annual primitive and cyclelen is a simple multiplier over a
##   fixed horizon (default --horizon=100 --cyclelen=1 -> 100 annual steps). Diameter growth is
##   the annual increment * cyclelen; survival is annual^cyclelen; the stand-level transitions use
##   exp(-rate*cyclelen) and (1-exp(-k*cyclelen)). Set --cyclelen=5 to step in 5yr blocks.
##   STAND-LEVEL CONSTRAINT (replaces the ad hoc FIX 3 ramp). Two fitted stand-level equations
##   from stand_state_v3_production.R discipline the tree-level sum (Ritchie-Hann disaggregation:
##   the stand equation sets the total, the tree equations set the distribution):
##     - DENSITY. Fitted self-thinning rate r(RD) = ST_RATE * RD^ST_EXP per year (RD = SDI/SDImax,
##       metric). Per-tree natural hazards are rescaled by one stand-level scalar so the surviving
##       stems exactly match N1*exp(-r*cyclelen); the tree hazard sets WHICH trees die, the fitted
##       curve sets HOW MANY. This is the principled replacement for the hand-tuned SDImax ramp.
##     - BASAL AREA. Fitted carrying capacity Gmax = BA_G0 + BA_G1*SDImax (metric). Tree diameter
##       increments are scaled so the stand BA tracks the envelope BA1 + (Gmax-BA1)(1-exp(-k*dt)),
##       capping growth as BA approaches the size-density ceiling.
##   Disable either with --noconstrain (reverts to the v2 ad hoc ramp) for A/B comparison.
##
## v2 carried the THREE FIXES on top of the validated v1 engine:
...
NSTAND<-as.integer(ga("nstands","0")); SEED<-as.integer(ga("seed","7"))
## ANNUALIZED: fixed horizon, cyclelen is a simple multiplier of the step size.
...
ST_RATE<-as.numeric(ga("st_rate","0.34")); ST_EXP<-as.numeric(ga("st_exp","2.5"))
## FIX A: density-dependent ingrowth taper params (canopy closure shuts off overstory recruitment)
IG_RDCAP<-as.numeric(ga("ig_rdcap","0.85")); IG_P<-as.numeric(ga("ig_p","1.5"))
## FIX B (optional): scale self-thin rate with site productivity (1=off). r_self *= (SDImax/SDIMAX_REF)^st_site_scale
ST_SITE_SCALE<-as.numeric(ga("st_site_scale","0.0")); SDIMAX_REF<-as.numeric(ga("sdimax_ref","935"))
...
cat("==== conus_eq_projector_v3 (annualized + stand-level constraint) ====\n"); cat(sprintf("  variant=%s mode=%s (CONFIG=%s) nstands=%s horizon=%dyr cyclelen=%d -> %d steps\n",VARIANT,MODE,CONFIG,if(NSTAND>0)NSTAND else "ALL",HORIZON,CYCLEN,NCYC))
cat("  WIRED: DG(b2/b1), mortality(logit/gompit), ht-dbh(Wykoff), CR-recession, dynamic competition.\n")
if(CONSTRAIN) cat(sprintf("  STAND-LEVEL CONSTRAINT ON: density self-thin r=%.4f*RD^%.2f/yr; BA cap Gmax=%.2f+%.4f*SDImax, k=%.4f/yr.\n\n",ST_RATE,ST_EXP,BA_G0,BA_G1,BA_K)) else cat("  STAND-LEVEL CONSTRAINT OFF: reverting to v2 ad hoc SDImax ramp.\n\n")
...
TEMPORAL_FLOOR <- as.logical(as.integer(Sys.getenv("TEMPORAL_FLOOR","1")))  # v6 FIX D2 default ON
...
for(stcode in names(state_groups)){
  ...
  for(ix in state_groups[[stcode]]){
    ...
    emit<-function(cy){sm<-stand_metrics(tl); py<-cy*CYCLEN; ri<<-ri+1
      ...
      ## v4 FIX C: save REAL projected height. HT_M_MEAN = TPA-wtd mean; HT_M_DOM = top height
      ## (TPA-wtd mean HT of the largest-DBH stems accumulating to 100 stems/ha = ~40.5 tpa).
      htm<-NA_real_; htd<-NA_real_
      if(length(tl$HT)>0){ okh<-is.finite(tl$HT)&is.finite(tl$TPA)&tl$TPA>0
        if(any(okh)){ htm<-sum(tl$HT[okh]*tl$TPA[okh])/sum(tl$TPA[okh])
          ordh<-order(tl$dbh_in[okh],decreasing=TRUE); hh<-tl$HT[okh][ordh]; tt<-tl$TPA[okh][ordh]
          target<-100/2.4710538; cum<-cumsum(tt); ndom<-which(cum>=target)[1]; if(is.na(ndom)) ndom<-length(tt)
          wdom<-tt[seq_len(ndom)]; if(ndom>=1){ wdom[ndom]<-wdom[ndom]-(cum[ndom]-target); wdom<-pmax(wdom,0); if(sum(wdom)>0) htd<-sum(hh[seq_len(ndom)]*wdom)/sum(wdom) } } }
      rows[[ri]]<<-data.table(STAND_CN=cn,STATE=st,YEAR=inv_year+py,PROJ_YEAR=py,VARIANT=VARIANT,CONFIG=CONFIG,AGB_TONS_AC=NA_real_,BA_FT2AC=sm$BA,QMD_IN=sm$QMD,TPH=sm$TPH,HT_M_MEAN=htm,HT_M_DOM=htd,CCH_MEAN=cchm)
    ...
    for(cy in 1:NCYC){
      ...
      ## --- STAND-LEVEL BA CONSTRAINT: scale the increment so stand BA tracks the Gmax envelope ---
      if(CONSTRAIN){
        sdimax_m<-tl$SDImax[1]; ba_pre_m<-tl$BA*0.2296
        dbh_nat<-pmin(pmax(tl$dbh_in+dg_a*CYCLEN,0.1),200)
        ba_post_m<-sum(pi/4*dbh_nat^2/144*tl$TPA)*0.2296
        gmax_m<-BA_G0+BA_G1*sdimax_m
        ba_env_m<-ba_pre_m+(gmax_m-ba_pre_m)*(1-exp(-BA_K*CYCLEN))
        inc_nat<-ba_post_m-ba_pre_m; inc_env<-ba_env_m-ba_pre_m
        f_ba<-if(inc_nat>1e-9) min(max(inc_env/inc_nat,0),3) else 1
        dg_use<-dg_a*f_ba
      } else dg_use<-dg_a
      tl$dbh_in<-pmin(pmax(tl$dbh_in+dg_use*CYCLEN,0.1),200)
      HT2<-ht_from_dbh(tl)*tl$ht_ratio; HT2<-mono_ht(tl$dbh_in,HT2)  # v6 FIX D (isotonic-in-DBH)
      ...
      tl<-recompute_comp(tl)                       # competition reflects post-growth state
      ## --- per-tree NATURAL survival over the step (hazard form), no density ramp ---
      if(MORTMODE=="gompit"){
        gg<-grp_organon(tl$SPCD)
        cch<-stand_cch(tl$dbh_in, tl$HT/0.3048, tl$CR, tl$TPA, gg); tl$cch_last<-cch
        s_nat<-gomp_surv(tl$SPCD, tl$CR, cch, CYCLEN)
      } else {
        p_die_a<-1/(1+exp(-eta_mort(tl))); p_die_a[!is.finite(p_die_a)]<-0
        s_nat<-(1-pmin(pmax(p_die_a,0),0.999))^CYCLEN
      }
      s_nat<-pmin(pmax(s_nat,1e-9),1); H<- -log(s_nat)            # cycle hazard per tree
      N1<-sum(tl$TPA)
      if(CONSTRAIN && N1>0){
        ## DENSITY CONSTRAINT: rescale the per-tree hazards by ONE stand scalar so the surviving
        ## stems equal the fitted self-thinning total N1*exp(-r(RD)*cyclelen) (metric RD). The tree
        ## hazard sets which trees die; the fitted stand curve sets how many.
        qmd_in<-sqrt(sum(tl$dbh_in^2*tl$TPA)/N1); tph<-N1*2.4710538
        sdi_m<-tph*(qmd_in*2.54/25)^1.605; rd_m<-sdi_m/max(tl$SDImax[1],1)
        r_self<-ST_RATE*rd_m^ST_EXP
        if(ST_SITE_SCALE!=0) r_self<-r_self*(max(tl$SDImax[1],1)/SDIMAX_REF)^ST_SITE_SCALE
        N_target<-N1*exp(-r_self*CYCLEN)
        if(N_target<N1-1e-6 && any(H>1e-9)){
          fm<-function(m) sum(tl$TPA*exp(-H*m))-N_target
          m<-tryCatch(if(fm(0.1)>0 && fm(50)<0) uniroot(fm,c(0.1,50))$root else 1, error=function(e) 1)
          surv<-exp(-H*m)
        } else surv<-s_nat
      } else {
        rd<-tl$rd_add; st_mult<-pmin(1+pmax(rd-0.55,0)/0.45*2.0,8)   # v2 ad hoc ramp (--noconstrain)
        surv<-exp(-H*st_mult)
      }
      ...
      ## ingrowth: v4 FIX A -- density-DEPENDENT. Taper the empirical per-acre recruit rate to 0 as the
      ## canopy closes (relative density RD=SDI/SDImax). Closed stands do not recruit into the overstory.
      ## RD is evaluated on the post-mortality stand (current tl after survival applied).
      N_ig<-sum(tl$TPA)
      if(N_ig>0){
        qmd_ig<-sqrt(sum(tl$dbh_in^2*tl$TPA)/N_ig); tph_ig<-N_ig*2.4710538
        sdi_ig<-tph_ig*(qmd_ig*2.54/25)^1.605; rd_ig<-sdi_ig/max(tl$SDImax[1],1)
      } else rd_ig<-0
      f_ig<-if(rd_ig>=IG_RDCAP) 0 else max(0,(1-rd_ig/IG_RDCAP))^IG_P
      if(nchar(Sys.getenv("TRACE_RECRUITS"))>0 && identical(cn, Sys.getenv("TRACE_RECRUITS")))
        cat(sprintf("[TRACE] cn=%s cy=%d RD=%.3f f_ig=%.3f recruits=%.3f TPH_pre=%.1f\n", cn, cy, rd_ig, f_ig, IG_TPA*CYCLEN*f_ig, sum(tl$TPA)*2.4710538))
      tl<-add_recruits(tl, IG_TPA*CYCLEN*f_ig)
...
## realized self-thinning slope diagnostic (Reineke expectation about -1.605)
od<-out[is.finite(QMD_IN)&is.finite(TPH)&TPH>0&QMD_IN>0]; setorder(od,STAND_CN,PROJ_YEAR)
sl<-od[,{lnN<-log(TPH);lnD<-log(QMD_IN); if(.N>=4 && var(lnD)>0 && last(TPH)<first(TPH)*0.98) as.numeric(cov(lnN,lnD)/var(lnD)) else NA_real_},by=STAND_CN]$V1
cat(sprintf("\nRealized self-thinning slope d lnN/d lnQMD: median=%.2f (Reineke ~ -1.605); self-thinning stands=%.0f%%  [constraint=%s]\n",
  median(sl,na.rm=TRUE), 100*mean(!is.na(sl)), if(CONSTRAIN)"ON" else "OFF"))
cat("DONE.\n")
