// ============================================================================
// hg_organon_speciesfree_v6_bgi.stan
//
// Height growth (ORGANON form) species-free B1, v6 architecture.
//
// MOTIVATION: First-principles audit (2026-05-24) found v5 fails the
// open-grown tree test (Weiskittel et al. 2011 Ch 15.2.3). Predictions
// are flat-to-rising across DBH 1-150 cm because a1*ln(DBH) outpaces
// a2*ln(HT) when HT saturates against site potential. The original
// ORGANON achieves asymptotic behavior via the GEA structure tied to
// a dominant-height growth equation; v5 lost this when going species-free.
//
// v6 FIX: Add a2_quad * ln(HT)^2 term with prior concentrated on negative
// values. This is the minimal surgical change that restores the missing
// asymptotic constraint while keeping the species-free architecture and
// every other v5 component intact.
//
// Mathematical justification: with a2 = -0.277 and a2_quad ~ -0.05 to
// -0.10, the marginal d(eta)/d(ln HT) becomes (a2 + 2*a2_quad*ln(HT)),
// which monotonically decreases in ln(HT) and crosses zero at finite
// height. This gives the model a built-in maximum.
//
// Linear predictor (v6, change vs v5 marked with *):
//   ln(HTG_a) = a0 + trait_effect[sp]
//             + z_L1 + z_L2 + z_L3 + z_FT
//             + a1 ln(DBH) + a2 ln(HT) + a2_quad ln(HT)^2 *  + a3 ln_cr_adj
...
transformed data {
  ...
  vector[N_obs] bgi_b2;
  vector[N_obs] bgi_b3;
  vector[N_obs] ln_ht_sq;        // v6: precompute quadratic
  for (i in 1:N_obs) {
...
parameters {
  ...
  real a2_quad;             // v6: NEW quadratic on ln(HT)
  real a3;
  real a4;
...
model {
  ...
  a1      ~ normal( 0.0, 0.5);
  a2      ~ normal(-0.3, 0.5);
  a2_quad ~ normal(-0.05, 0.05);   // v6: prior concentrated negative
  a3      ~ normal( 0.5, 0.5);
  a4      ~ normal( 0.05, 0.1);
  ...
  vector[N_obs] eta =
    ...
    + a1 * ln_dbh
    + a2 * ln_ht + a2_quad * ln_ht_sq   // v6: quadratic asymptote
    + a3 * ln_cr_adj
    + b_site .* bgi_b1
