...
p1y <- function(trees, mortcal, ingrowth="Y", cutpoint=NULL, csi_scale=NULL, csi_per_stand=NULL) {
  pc <- list()
...
run_cfg <- function(mortcal, ingrowth="Y", cutpoint=NULL, csi_scale=NULL, csi_per_stand=NULL) {
  cur <- base_init; bl <- list(); tl <- list(); ql <- list()
  bl[["0"]] <- basnap(cur); tl[["0"]] <- tpsnap(cur); ql[["0"]] <- qmsnap(cur)
  for (yr in 1:n_years) {
    cur <- p1y(cur, mortcal, ingrowth, cutpoint, csi_scale, csi_per_stand); if (is.null(cur)) break
