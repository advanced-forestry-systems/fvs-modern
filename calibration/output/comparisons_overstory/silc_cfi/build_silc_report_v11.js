...
children.push(new Paragraph({
  ...
  children: [new TextRun({ text: "Companion Report v11 — AcadianGY clarification (AGM unrun)", italics: true, size: 26 })],
}));
children.push(new Paragraph({
...
children.push(p("Cardinal job 11076533 ran AcadianGY 12.3.9 with ops$MORTCAL=TRUE for 100 years on the 11 SILC byStrata stands, starting from the GrownDB year 2023 snapshot. The default AcadianGY trajectory has every stratum approaching ~200 ft^2/ac at year 100, an artifact of the under mortality issue. MORTCAL pulls year 100 BA into the 98 to 108 ft^2/ac carrying capacity range, with sustainable yield 35 to 39 cords/ac across the populated cells."));
...
children.push(p("Side by side year 100 BA in ft^2/ac on the same 11 byStrata starting stands plus CFI backfill on 2 of 4 empty cells. AcadianGY MORTCAL anchors the operational floor; FVS-NE calibrated and OSM-ACD cluster as the upper bracket."));
...
children.push(bullet("AcadianGY with MORTCAL on is the new long horizon champion. Use it for SILC operational sawlog projections (BdFt R^2 = 0.84) and Cords (R^2 = 0.82). Set ops$MORTCAL = TRUE, MORTCAL_INTERVAL = 5 in any AcadianGY call."));
...
children.push(bullet("OSM-ACD is the conservative under projection (-9.7% BA bias at 10 yr). Report AcadianGY MORTCAL plus OSM as the operational range; per plot RMSE 10.7 ft^2/ac BA at 10 yr horizon is the empirical uncertainty band."));
...
const out = `${CFI_DIR}/SILC_4Model_Benchmark_Report_v11_Weiskittel.docx`;
