...
// ===================================================================
{
  ...
  // Year-100 outcomes table: default vs MORTCAL side by side
  const HDRY = { bold: true, color: "FFFFFF", fill: { color: CRSF_GREEN } };
  const rows = [
    [{ text: "Stratum",                  options: HDRY },
     { text: "n",                        options: HDRY },
     { text: "BA default",               options: HDRY },
     { text: "BA MORTCAL",               options: HDRY },
     { text: "delta BA",                 options: HDRY },
     { text: "Cords default",            options: HDRY },
     { text: "Cords MORTCAL",            options: HDRY },
     { text: "delta Cords",              options: HDRY }],
    [{ text: "Cedar / A+B (CFI backfill)", options: { fill: { color: "FFF8E1" }, italic: true } },
    ...
    [{ text: "Mixedwood / A+B (CFI backfill)", options: { fill: { color: "FFF8E1" }, italic: true } },
    ...
    [{ text: "Other Softwood (no CFI coverage)", options: { fill: { color: "F4F4F4" }, italic: true, color: "888888" } },
                                   "0", "—", "—",   "—",   "—",   "—",   "—"]
  ];
  s.addTable(rows, {
    x: 0.3, y: 1.65, w: 12.7, h: 4.5,
    colW: [3.0, 0.6, 1.3, 1.4, 1.3, 1.4, 1.5, 1.4],
  ...
  citationLine(s, "Default vs MORTCAL=TRUE on 11 byStrata + 4 CFI backfill plots. BA in ft^2/ac, Cords/ac. Other Softwood has no CFI coverage.");
  takeawayBand(s, "MORTCAL year-100 BA is half the default; CFI backfill closes 2 of 4 empty cells");
}
...
// ===================================================================
{
  ...
  const recs = [
    { title: "AGM with MORTCAL correction is the new long horizon champion",
      body: "Cuts AGM BA bias from +18.5% to +7.8% (10 yr empirical), and pulls 100 yr projected BA from ~200 down to ~100 ft^2/ac and 100 yr Cords from ~62 down to ~37 cords/ac. Sawlog BdFt R^2 hits 0.84 with single digit bias. Recommended for SILC operational sawlog and yield projections.",
      accent: CRSF_GREEN },
...
const out = `${CFI_DIR}/SILC_4Model_Benchmark_v8_Weiskittel.pptx`;
