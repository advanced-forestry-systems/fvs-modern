...
// ===================================================================
{
  const s = pres.addSlide();
  s.background = { color: "FFFFFF" };
  s.addText("Important clarification: AGM and AcadianGY are different models", {
    x: 0.5, y: 0.35, w: 12.3, h: 0.85,
    fontFace: "Aptos Display", fontSize: 28, color: CHARCOAL, bold: true,
    align: "left", valign: "middle"
  });
  s.addText("Three models are benchmarked in this deck. SILC's operational AGM is NOT one of them yet.", {
...
// ===================================================================
{
  ...
  s.addText("Default AcadianGY (green) projects BA climbing to 200 ft^2/ac by year 100. MORTCAL=TRUE (gold) plateaus at 80-110 ft^2/ac - a more operationally credible carrying capacity for these stands.", {
...
// ===================================================================
{
  ...
  citationLine(s, "Year-100 (2123) BA in ft^2/ac, Cords/ac. AcadianGY 12.3.9; FVS-NE job 11078908; OSM-ACD job 11091396 (5 of 6 cells; H3B-N + H3C-N still failing — Hardwood A+B uncovered)");
...
// ===================================================================
{
  ...
  s.addText("v3 of the SILC CFI database includes approximate lat/long (46.4628 N, 68.4253 W, Davistown). Used to sample regional rasters and replace defaults.", {
    x: 0.5, y: 1.05, w: 12.3, h: 0.55,
    fontFace: "Aptos", fontSize: 14, color: MED_GRAY, italic: true,
    align: "left", valign: "top"
  });
...
// ===================================================================
{
  ...
  const recs = [
    ...
    { title: "AcadianGY with MORTCAL is the closest CRSF analog for an operational anchor",
      body: "Cuts 10 yr empirical BA bias from +18.5% to +7.8%; year 100 BA settles at 98 to 108 ft^2/ac, Cords 35 to 39 cords/ac. Sawlog BdFt R^2 = 0.84 with single digit bias. Until SILC AGM is benchmarked, use AcadianGY MORTCAL as the operational floor.",
      accent: CRSF_GREEN },
    { title: "FVS-NE calibrated brackets the upper bound on year 100 and is best on short horizon",
      body: "Year 100 BA 125 to 225 ft^2/ac, Cords 44 to 70. Sits between AcadianGY default and MORTCAL on every cell. Lowest 5 yr CFI bias (-0.3% BA, -3.8% Cords). Use as the FVS variant cross check.",
      accent: CRSF_GREEN },
    { title: "OSM-ACD as the additional bracket",
      body: "OSM-ACD lands at year 100 BA 174 to 249 ft^2/ac, Cords 32 to 42 across covered cells, sitting close to FVS-NE calibrated. Report AcadianGY MORTCAL plus FVS-NE cal plus OSM-ACD as the range until SILC AGM is added.",
      accent: CRSF_GREEN }
  ];
...
// ===================================================================
{
  ...
  const mechs = [
    ...
    { lab: "Verified, not a calculation issue:",
      body: "BA / Cords / BdFt pipeline reproduces from the AcadianGY treelist to 1e-13 ft^2/ac. The bias is the model speaking honestly." }
  ];
  mechs.forEach((m, i) => {
    const y = 5.65 + i * 0.45;
    s.addText(`• ${m.lab}`, {
...
// ===================================================================
{
  ...
  s.addText("The #126b in-source size dependent mortality correction (calibrated to Maine FIA) ports cleanly to SILC CFI. Default OFF in AcadianGY 12.3.9; turn on by setting ops$MORTCAL = TRUE", {
    x: 0.5, y: 1.05, w: 12.3, h: 0.55,
    fontFace: "Aptos", fontSize: 14, color: MED_GRAY, italic: true,
    align: "left", valign: "top"
  });
...
// ===================================================================
{
  ...
  const steps = [
    ...
    { n: "3", t: "Validate year 100 BA against SILC operational expectations", b: "MORTCAL 100 yr BA settles at 80 to 110 ft^2/ac. Confirm this matches SILC's expected carrying capacity for managed Acadian stands" },
    { n: "4", t: "Set the uncertainty bands and ship the operational handoff", b: "Use AcadianGY MORTCAL RMSE 10.7 ft^2/ac BA at 10 yr as the per plot uncertainty. Year 100 cords/ac estimates: 36 to 39 across strata" }
  ];
...
const out = `${CFI_DIR}/SILC_4Model_Benchmark_v11_Weiskittel.pptx`;
