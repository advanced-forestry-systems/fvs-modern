#!/usr/bin/env Rscript
# run_acadiangy_long_mortcal.R
# AcadianGY long-horizon driver with the #126b in-source MORTCAL=TRUE
# size-dependent mortality correction enabled. Everything else identical
# to run_acadiangy_long.R.
...
for (i in seq_len(nrow(manifest))) {
  ...
  ops <- list(verbose=FALSE, INGROWTH="Y", MinDBH=5.0,
              MORTCAL=TRUE, MORTCAL_INTERVAL=5)  # <-- #126b in-source patch on
...
write.csv(do.call(rbind, out_rows),
          file.path(WD, "silc_cfi_long_agy_mortcal_results.csv"), row.names=FALSE)
cat("\nDone (MORTCAL=TRUE)\n")
