...
children.push(new Paragraph({
  ...
  children: [new TextRun({ text: "Companion Report v10 — 4-model year 100 + refined inputs", italics: true, size: 26 })],
}));
children.push(new Paragraph({
...
children.push(bullet("(4) Full 4 model year 100 view added: FVS-NE default 209 to 291 BA, FVS-NE calibrated 125 to 225, OSM-ACD 172 to 221 (covered cells). AGM MORTCAL still the lowest projection. Refined inputs from CFI v3 lat/long swap BGI 3000 to 3902 and CSI 12 to 15.78 m; minimal effect on relative ranking."));
children.push(p("Recommended operational stack: AGM with MORTCAL on as the long horizon operational anchor (35 to 39 cords/ac year 100); FVS-NE calibrated as the upper bracket (44 to 70 cords/ac); OSM-ACD as additional cross check where coverage exists."));
...
children.push(p("Side by side year 100 BA in ft^2/ac on the same 11 byStrata starting stands plus CFI backfill on 2 of 4 empty cells. AGM MORTCAL anchors the operational floor; FVS-NE calibrated and OSM-ACD cluster as the upper bracket."));
const y100Hdr = ["Stratum", "n", "AGM def", "AGM mc", "FVS-NE def", "FVS-NE cal", "OSM-ACD"];
const y100Rows = [
...
const cHdr = ["Stratum", "n", "AGM MORTCAL", "FVS-NE cal", "OSM-ACD"];
const cRows = [
...
const out = `${CFI_DIR}/SILC_4Model_Benchmark_Report_v10_Weiskittel.docx`;
