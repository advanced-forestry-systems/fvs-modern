...
if ("CCH2" %in% names(dat)) dat[, delta_cch := as.numeric(CCH2) - cch] else dat[, delta_cch := 0]; dat[!is.finite(delta_cch), delta_cch := 0]
stan_data <- list(
...
if (SMOKE) { iw <- 50; is_ <- 50; ch <- 2 } else { iw <- 1500; is_ <- 1000; ch <- 4 }

t0 <- Sys.time()
fit <- mod$sample(data = stan_data, chains = ch, parallel_chains = ch,
                  iter_warmup = iw, iter_sampling = is_,
                  adapt_delta = 0.95, max_treedepth = 12, seed = 2026, refresh = 100)
