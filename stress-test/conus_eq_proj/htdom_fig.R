...
OUT<-'/fs/scratch/PUOM0008/crsfaaron/fvs_stress/conus_eq_proj/bakuzis_singlecohort/v5_figs'
...
png(file.path(OUT,'htdom_recession_v4_v5.png'),width=1500,height=520,res=110)
par(mfrow=c(1,3),mar=c(4,4,3,1))
for(rg in c('NE','SN','PN')){
  a4<-agg_arm('/fs/scratch/PUOM0008/crsfaaron/fvs_stress/conus_eq_proj/out_v4_seed',rg)
  a5<-agg_arm('/fs/scratch/PUOM0008/crsfaaron/fvs_stress/conus_eq_proj/out_v5_seed',rg)
  yl<-range(c(a4$HTDOM,a5$HTDOM),na.rm=TRUE)
  plot(NA,xlim=c(0,140),ylim=yl,xlab='cohort age (yr)',ylab='top height HT_M_DOM (m)',main=paste0(rg,': top-height recession (v4 solid, v5 dashed)'))
  cols<-c('1'='#762a83','2'='#1b7837','3'='#2166ac','4'='#b35806')
  for(s in 1:4){d4<-a4[site==s];d5<-a5[site==s]
    lines(d4$agebin,d4$HTDOM,col=cols[as.character(s)],lwd=2)
    lines(d5$agebin,d5$HTDOM,col=cols[as.character(s)],lwd=2,lty=2)}
  legend('bottomright',legend=paste('site',1:4),col=cols,lwd=2,bty='n',cex=0.8)
}
dev.off(); cat('htdom figure written\n')
