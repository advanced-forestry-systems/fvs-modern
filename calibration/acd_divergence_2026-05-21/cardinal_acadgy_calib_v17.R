...
cal <- read.csv(file.path(PROJECT_ROOT,"fvs-modern-acdbridge/calibration/acd_divergence_2026-05-21/acd_annual_calibration.csv"))
cal_dia <- setNames(cal$dDBH.mult, cal$SP); cal_ht <- setNames(cal$dHt.mult, cal$SP)
...
p1y <- function(trees, mortcal, calib=FALSE) {
  pc <- list()
  for (sid in unique(trees$STAND)) {
    ...
    if (calib) { dm<-cal_dia[sub$SP]; hm<-cal_ht[sub$SP]; sub$dDBH.mult<-ifelse(is.na(dm),1,as.numeric(dm)); sub$dHt.mult<-ifelse(is.na(hm),1,as.numeric(hm)) }
...
rows <- list()
cat("[v17] canonical_off  (no calib, MORTCAL off)\n");  rows[["off"]]   <- summ("canonical_off",  run_cfg(FALSE, FALSE))
cat("[v17] calibrated_off (dia calib,  MORTCAL off)\n"); rows[["cal"]]   <- summ("calibrated_off", run_cfg(FALSE, TRUE))
cat("[v17] calibrated_on  (dia calib + MORTCAL)\n");     rows[["calon"]] <- summ("calibrated_on",  run_cfg(TRUE,  TRUE))
res <- dplyr::bind_rows(rows); write.csv(res, file.path(OUT_DIR, "acadgy_calib_v17_results.csv"), row.names=FALSE)
cat("\n=== FIA: does diameter calibration help or hurt stand BA? ===\n"); print(format(res, digits=4))
