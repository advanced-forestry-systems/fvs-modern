...
rows <- list()
cat("[v19] cp95_mort   (CutPoint 0.95, MORTCAL on)\n");  rows[["a"]] <- summ("cp95_mort",   run_cfg(TRUE,  "Y", 0.95))
cat("[v19] cp50_mort   (CutPoint 0.50, MORTCAL on)\n");  rows[["b"]] <- summ("cp50_mort",   run_cfg(TRUE,  "Y", 0.50))
cat("[v19] cp50_nomort (CutPoint 0.50, MORTCAL off)\n"); rows[["c"]] <- summ("cp50_nomort", run_cfg(FALSE, "Y", 0.50))
res <- dplyr::bind_rows(rows); write.csv(res, file.path(OUT_DIR, "acadgy_cutpoint_v19_results.csv"), row.names=FALSE)
cat("\n=== FIA: does enabling ingrowth (lower CutPoint) fix the QMD/BA residual? ===\n"); print(format(res, digits=4))
