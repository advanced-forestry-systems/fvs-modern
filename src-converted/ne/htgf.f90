...
INTEGER JJ, MFORM
! --- RELHTA competition-decoupling experiment (defaults reproduce stock behaviour) ---
REAL HRLA_ALPHA, HRLA_CAP, HRLA_GK
LOGICAL HRLA_DONE
CHARACTER*80 HRLA_CV
SAVE HRLA_ALPHA, HRLA_CAP, HRLA_GK, HRLA_DONE
DATA HRLA_DONE/.FALSE./
!
!----------
!  SEE IF WE NEED TO DO SOME DEBUG.
!----------
CALL DBCHK (DEBUG,'HTGF',4,ICYC)
IF (.NOT.HRLA_DONE) THEN
  HRLA_ALPHA=1.0
  HRLA_CAP=1.0
  HRLA_GK=0.8
  CALL GETENV('FVS_HTG_RELHTA_ALPHA', HRLA_CV)
  IF (HRLA_CV.NE.' ') READ(HRLA_CV,*,ERR=8801) HRLA_ALPHA
8801 CONTINUE
  HRLA_CV=' '
  CALL GETENV('FVS_HTG_RELHTA_CAP', HRLA_CV)
  IF (HRLA_CV.NE.' ') READ(HRLA_CV,*,ERR=8802) HRLA_CAP
8802 CONTINUE
  HRLA_CV=' '
  CALL GETENV('FVS_HTG_GMODK', HRLA_CV)
  IF (HRLA_CV.NE.' ') READ(HRLA_CV,*,ERR=8803) HRLA_GK
8803 CONTINUE
  HRLA_DONE=.TRUE.
  IF (HRLA_ALPHA.NE.1.0 .OR. HRLA_CAP.NE.1.0 .OR. HRLA_GK.NE.0.8) &
    WRITE(JOSTND,*) 'HTGF RELHTA CONTROLS: ALPHA=',HRLA_ALPHA, &
       ' CAP=',HRLA_CAP,' GMODK=',HRLA_GK
ENDIF
...
IF (LGREGHG) THEN
  DO JJ=1,ITRN
    ...
    IF (GHAVE_MCW(ISP(JJ))) THEN
      ...
      MCWMAX=GMCW(ISP(JJ),5); MCWDMX=GMCW(ISP(JJ),6)
    ELSE
      MFORM=1; MCWA=8.0; MCWB=1.5; MCWC=0.0; MCWMAX=0.0; MCWDMX=0.0
    ENDIF
    DBHEFF = DBH(JJ)
...
!      GMOD = (GMOD+1.0)/2.0
RELHTA=0.
IF(AVH .GT. 0.)RELHTA=MIN((HT(I)/AVH),HRLA_CAP)
GMOD=(1.0-((1.0-GMOD)*(1.0-HRLA_ALPHA*RELHTA)))*HRLA_GK
