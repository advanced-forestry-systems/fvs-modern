...
!----------
!  GOMPIT: recompute per-tree crown closure at tip for this cycle.
!----------
...
!----------
!  GOMPIT OVERRIDE: fitted species use gompit (density-aware via crown closure
!  at tip); unfit species fall back to background rate only. VARMRT skipped
!  below when LGOMP.
!----------
IF(LGOMP) THEN
  IF(GHAVE(ISPC)) THEN
    ...
    CCHG=CCHT(I)
    CALL GOMPSURV(ISPC,DBH(I),CRG,CCHG,FINT,SURVG)
    WKI=P*(1.0-SURVG)
  ELSE
...
! GOMPIT owns density mortality via crown closure; skip native redistribution.
