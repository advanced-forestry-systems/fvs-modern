// build_silc_deck_v3.js
// =====================================================================
// SILC operational deck v3 for Ian / Ryan at Seven Islands.
// Extends v2 with: International 1/4 sawlog BdFt for all 7 predictors,
// SDI (Reineke) and Curtis RD scorecards, observed CFI species
// composition with OSM-ACD comparison.
// =====================================================================
...
// ===================================================================
{
  const s = pres.addSlide();
  s.background = { color: "FFFFFF" };
  s.addText("Species composition tracks well over time", {
    x: 0.5, y: 0.35, w: 12.3, h: 0.75,
    fontFace: "Aptos Display", fontSize: 32, color: CHARCOAL, bold: true,
    align: "left", valign: "middle"
  });
  s.addText("Comm SW share grew from 43% to 53% between 1990 and 2000, OSM-ACD captures the shift", {
    x: 0.5, y: 1.05, w: 12.3, h: 0.45,
    fontFace: "Aptos", fontSize: 16, color: MED_GRAY, italic: true,
    align: "left", valign: "top"
  });
  ...
  s.addImage({
    path: `${CFI_DIR}/silc_cfi_species_composition_deck.png`,
    x: 0.3, y: 1.50, w: 12.7, h: 4.50
  });

  citationLine(s, "Observed: CFI TREE.csv per measurement year (mean across plots). OSM-ACD: TreeListProjections at year_curr. AGM and FVS per tree species output not enabled in current drivers");
  takeawayBand(s, "OSM-ACD species share within 2.5 pp of observed across all four groups");
}
...
// ===================================================================
{
  ...
  const steps = [
    ...
    { n: "2", t: "Enable AGM and FVS per tree species output", b: "Adds species composition for those two models, so the comp scorecard is four model instead of OSM only" },
...
const out = `${CFI_DIR}/SILC_4Model_Benchmark_v3_Weiskittel.pptx`;
