#!/usr/bin/env Rscript
# dg_site_driver_variants_v2.R
# Complete the DG site-driver family with the CLIMATE (EMT) variant, now that
# the P11 crosswalk gives EMT for all pairs. All drivers standardized (mean0/sd1)
# so the comparison is clean and BGI no longer overflows.
#   dg_annual = exp(B0 + B1*log((dbh+1)^2/(cr*ht+1)^B3) + B2*bal^B4/log(dbh+2.7) [+ B5*Z])
#   driver Z in { none, elev, bgi, cspi, esi(=climate_si), emt(ClimateNA, via crosswalk) }
# Single-step annual approx, applied identically to every variant. Per species,
# 70/30 held-out. Author: Cowork autopilot 2026-07-03. Seed 20260703.

suppressPackageStartupMessages({ library(data.table); library(minpack.lm) })
set.seed(20260703)
OUT <- "/fs/scratch/PUOM0008/crsfaaron/fvs-conus_output_conus/dg_site_variants_v2"
dir.create(OUT, recursive = TRUE, showWarnings = FALSE)
CFG <- "/users/PUOM0008/crsfaaron/fvs-modern/config"
rmse <- function(a,b) sqrt(mean((a-b)^2)); bias <- function(a,b) mean(a-b)

drivers <- c("none","elev","bgi","cspi","esi","emt")
drv_col <- c(elev="ELEV", bgi="bgi", cspi="cspi", esi="esi", emt="EMT")
base_start <- c(B0=-1.326, B1=-0.57, B2=-0.05, B3=1.5, B4=0.8)
fit_driver <- function(dat, drv) {
  if (drv == "none") {
    frm <- dg_obs ~ exp(B0 + B1*log((dbh+1)^2/(cr*ht+1)^B3) + B2*bal^B4/log(dbh+2.7))
    st  <- as.list(base_start)
  } else {
    frm <- dg_obs ~ exp(B0 + B1*log((dbh+1)^2/(cr*ht+1)^B3) + B2*bal^B4/log(dbh+2.7) + B5*Z)
    st  <- as.list(c(base_start, B5 = 0))
  }
  tryCatch(nlsLM(frm, data=dat, start=st, control=nls.lm.control(maxiter=200)),
           error=function(e) NULL)
}

d <- as.data.table(readRDS("~/fvs-conus/calibration/data/conus_remeasurement_pairs.rds"))
# join EMT from the P11 crosswalk
xw <- fread("/fs/scratch/PUOM0008/crsfaaron/fvs-conus_output_conus/emt_crosswalk/plot_key_emt_td_crosswalk.csv")
d <- merge(d, xw[, .(plot_key, EMT)], by="plot_key", all.x=TRUE)
d <- d[is.finite(DBH1)&is.finite(DBH2)&is.finite(CR1)&is.finite(HT1)&is.finite(BAL1)&
       is.finite(YEARS)&YEARS>0 & CR1>0 & CR1<1 & DBH1>=1 & STATUS1==1 & STATUS2==1]
d[, dg_obs := (DBH2-DBH1)/YEARS]
d <- d[dg_obs > -0.1 & dg_obs < 2]
setnames(d, c("DBH1","CR1","HT1","BAL1"), c("dbh","cr","ht","bal"))
d[, esi := climate_si]
cat("rows:", format(nrow(d),big.mark=","), " EMT finite:", round(mean(is.finite(d$EMT)),3), "\n")

sp_n <- d[, .N, by=SPCD][N >= 3000]
score <- list()
for (drv in drivers) {
  co <- list(); te_all <- list()
  for (s in sp_n$SPCD) {
    ds <- d[SPCD==s]
    if (drv != "none") { ds[, Z := scale(get(drv_col[[drv]]))[,1]]; ds <- ds[is.finite(Z)] }
    if (nrow(ds) < 2000) next
    i <- sample(nrow(ds), floor(nrow(ds)*0.7)); tr <- ds[i]; te <- ds[-i]
    m <- fit_driver(tr, drv); if (is.null(m)) next
    p <- coef(m)
    co[[as.character(s)]] <- data.table(SPCD=s, n=nrow(ds), B0=p["B0"],B1=p["B1"],B2=p["B2"],
        B3=p["B3"],B4=p["B4"], B5=if("B5"%in%names(p)) p["B5"] else NA_real_)
    eta <- p["B0"]+p["B1"]*log((te$dbh+1)^2/(te$cr*te$ht+1)^p["B3"])+p["B2"]*te$bal^p["B4"]/log(te$dbh+2.7)
    if (drv!="none") eta <- eta + p["B5"]*te$Z
    # clamp eta so a held-out outlier (esp. BGI) cannot overflow exp() to Inf;
    # observed annual DG is filtered < 2 in/yr, so cap predictions at 6 in/yr.
    te_all[[as.character(s)]] <- data.table(obs=te$dg_obs, pred=exp(pmin(eta, log(6))))
  }
  CO <- rbindlist(co); TE <- rbindlist(te_all)
  fwrite(CO, file.path(CFG, sprintf("greg_dg_coefficients_%s_std.csv", drv)))
  score[[drv]] <- data.table(driver=drv, species=nrow(CO), n_test=nrow(TE),
     RMSE=rmse(TE$pred,TE$obs), bias=bias(TE$pred,TE$obs),
     r=suppressWarnings(cor(TE$pred,TE$obs)),
     med_abs_B5=if(drv=="none") NA_real_ else median(abs(CO$B5),na.rm=TRUE))
}
SC <- rbindlist(score)[order(RMSE)]
cat("\n=== DG site-driver scorecard v2 (held-out, in/yr; standardized drivers; incl EMT) ===\n"); print(SC)
fwrite(SC, file.path(OUT, "dg_site_variant_scorecard_v2.csv"))
cat("\nBenchmarks (six-arm held-out, in/yr): default 0.269 | recalibrated 0.134 | spp-specific 0.090 | spp-free 0.098\n")
