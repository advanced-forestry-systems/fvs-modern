...
CONSTRAIN<-tolower(ga("constrain","off")); stopifnot(CONSTRAIN %in% c("off","sdi"))
if(CONSTRAIN=="sdi") CONFIG<-paste0(CONFIG,"_sdicon")
RD_ON<-as.numeric(ga("rd_on","0.55")); RD_MAXMULT<-as.numeric(ga("rd_maxmult","40"))
...
recompute_comp<-function(tl){DBH<-tl$dbh_in; TPA<-tl$TPA; n<-length(DBH); ba_tree<-pi/4*DBH^2/144; BA_ac<-ba_tree*TPA; BA_tot<-sum(BA_ac); ord<-order(DBH,decreasing=TRUE)
  ...
  ## metric-convention SDI (TPH/ha, QMD cm, ref 25cm) to match SDImax_brms scale & Bakuzis harness
  sT<-sum(TPA); QMD_in<-if(is.finite(sT)&&sT>1e-6) sqrt(sum(DBH^2*TPA)/sT) else 0; QMD_cm<-QMD_in*2.54; TPH<-sT*2.4710538
  SDI_met<-if(QMD_cm>0) TPH*(QMD_cm/25)^1.605 else 0; rd_met<-SDI_met/pmax(tl$SDImax,1)
  tl$BA<-BA_tot; tl$BAL<-BAL; tl$BAL_SW<-BAL_SW; tl$BAL_HW<-BAL_HW; tl$SDI<-SDI; tl$rd_add<-rd_add; tl$sdi_cx<-sdi_cx; tl$SDI_met<-SDI_met; tl$rd_met<-rd_met; tl
...
for(stcode in names(state_groups)){
  ...
  for(ix in state_groups[[stcode]]){
    ...
    for(cy in 1:NCYC){
      ...
      rd<-tl$rd_add
      if(CONSTRAIN=="sdi"){
        ## STAND-LEVEL DENSITY CONSTRAINT: boundary-following self-thinning on the
        ## metric-convention relative density rd_met = SDI_met/SDImax_brms (same scale
        ## as SDImax_brms & the Bakuzis harness). Below RD_ON (~0.55, onset of
        ## competition mortality) no extra mortality; above it the multiplier ramps
        ## steeply so the stand tracks the self-thinning line and asymptotes near SDImax.
        rdm<-tl$rd_met; rdm[!is.finite(rdm)]<-0
        st_mult<-pmin(1+pmax(rdm-RD_ON,0)/pmax(1-RD_ON,1e-3)*RD_MAXMULT, RD_MAXMULT+1)
      } else {
        st_mult<-pmin(1+pmax(rd-0.55,0)/0.45*2.0, 8)   # FIX 3 (legacy soft cap, English rd_add)
      }
      if(MORTMODE=="gompit"){
