...
python3 run_gompit_projection.py --variant ne --n 6 \
  ...
  --treeinit-dir /fs/scratch/PUOM0008/crsfaaron/FIA_fresh/treeinit \
  --output out_gompit/ne_val.csv --num-cycles 20 --cycle-length 5 \
  --arms default,calibrated,posthoc,iterative > out_gompit/ne_val.log 2>&1
echo "EXIT=$?" >> out_gompit/ne_val.log
