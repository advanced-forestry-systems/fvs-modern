...
//
// Architecture variant 1: linear + quadratic site productivity term to
// capture curvilinear (saturating or hump-shaped) site response. Addresses
// the implausible negative linear site coefficient (b6 = -0.019 in v0).
//
// Linear predictor:
//   ln(dDBH_a) = b0 + trait_effect[sp] + z_L1 + z_L2 + z_L3
//              + b1 ln(DBH) + b2 DBH + b3 ln((CR+0.2)/1.2)
//              + b4 ln(BAL_SW+0.01) + b5 BAL_HW
//              + b6 ln(CSI) + b9 ln(CSI)^2     <-- new quadratic term
...
model {
  ...
  b6 ~ normal( 0.5, 0.5);        // linear site: prior pulls toward positive
  b7 ~ normal( 0.0, 0.02);
  b8 ~ normal( 0.0, 0.02);
  b9 ~ normal(-0.1, 0.3);        // quadratic site: prior allows saturation (negative)
